...
class UNet_Boclis(nn.Module):
    def __init__(self, in_chns, class_num, feature_chns=None, dropout=None,
                 bilinear=False, init_type='kaiming'):
        super(UNet, self).__init__()

        feature_chns = feature_chns or [16, 32, 64, 128, 256]
        dropout = dropout or [0.05, 0.1, 0.2, 0.3, 0.5]
        ...
        self.encoder = Encoder(params)
        self.decoder = Decoder(params)
        self.out_activation = nn.Softmax(dim=1) if class_num > 1 else nn.Sigmoid()

        self.pool = nn.AdaptiveAvgPool2d((1, 1))
        self.projection_head = ProjectionHead(in_dim=feature_chns[-1], proj_dim=128)

        if init_type == 'kaiming':
            kaiming_normal_init_weight(self)
        elif init_type == 'sparse':
            sparse_init_weight(self)

    def forward(self, x):
        features = self.encoder(x)
        x4 = features[-1]  # shape: (B, C, H, W)

        # segmentation branch
        seg_out = self.decoder(features)
        seg_out = self.out_activation(seg_out)

        # contrastive learning branch
        gap = self.pool(x4).squeeze(-1).squeeze(-1)  # shape: (B, C)
        proj_out = self.projection_head(gap)  # shape: (B, 128)

        return seg_out, proj_out
